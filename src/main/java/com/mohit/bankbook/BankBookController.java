...
public class BankBookController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String dateFrom = request.getParameter("bbDateFrom");
		String dateTo = request.getParameter("bbTo");
		...
		out.println("<p class='bankbook-category-head'>CashBook</p>");
		out.println("<div class='right-bankbook-form-container' style='width: 57vw; height: 55vh; margin: auto; background-color: #070642;'>");
		out.println("<form class='left-bankbook-form' action='CashBookController'>");
		out.println("<label for='bbDateFrom' class='bbDateFrom'>Date From</label>");
		out.println("<label for='bbTo' class='bbTo'>To</label>");
		out.println("<br>");
		out.println("<input type='text' name='bbDateFrom' id='bbDateFrom' placeholder='yyyy-mm-dd' required='required'>");
		out.println("<input type='text' name='bbTo' id='bbTo' placeholder='yyyy-mm-dd' required='required'>");
		...
		ArrayList<BankBookBean> al = cd.findBbByFTDate(dateFrom, dateTo);
		
		for(BankBookBean cb : al) {
		...
		out.println("<th colspan='2'>" + totalAmount + "</th>");
		out.println("</tr>");
		out.println("</table>");
		out.println("</div>");
		out.println("</div>");
