...
public class IncomeCategory extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			/*userId from AddIncomeController*/			
		...
		if(userId == 0) {
			...
			out.println("<p class='income-category-head'>Income Category</p>");
					
			out.println("<form action='AddIncomeCategory' class='right-income-form' >");
			out.println("<label for='incCN' class='incCatNme'>Category Name:</label>");
			out.println("<input type='text' name='incCatName' id='incCN' placeholder='enter category name' required='required'>");
			out.println("<br>");
			out.println("<label for='incCD' class='incCatDetails'>Category Details:</label>");
			out.println("<input type='text' name='incCatDetails' id='incCD' placeholder='enter category details' required='required'>");
			out.println("<br>");
			out.println("<input type='text' name='userId' id='incCN' value='" + userId + "' style='display: none;'>");
			out.println("<input id='AddIncome' type='submit' value='Add'>");
			out.println("</form>");
		...
		for(Income_CategoryBean icb: al) {
			...
			out.println("<td><a href='Income_Category_update?inc_catId=" + icb.getInc_CatId() + "'>Edit</a></td>");
			out.println("<td><a href='Income_Category_Delete?inc_catId=" + icb.getInc_CatId() + "'>Delete</a></td>");
			out.println("</tr>");
		}				
			out.println("</table>");
			out.println("</div>");
